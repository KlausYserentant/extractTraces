function [calibrationFilePath, slitOrientation] = uiCalibration()
% open a gui to calculate a new calibration and return path to resulting file
    ...
    if strcmpi(slitOrientation,'Full frame') && strcmpi(channelNumber, '3 channels')
            ch23Separator = 2;
            [ch1, ch2] = loadImages(movieFilePath, filterIndex, rotationAngle, slitOrientation,channelNumber, ch23Separator);
            transMat12 = calculateTransformation(ch1, ch2);
            ch23Separator = 3;
            [ch1, ch3] = loadImages(movieFilePath, filterIndex, rotationAngle, slitOrientation,channelNumber, ch23Separator);
            transMat13 = calculateTransformation(ch1, ch3);
            posRect = selectFieldOfView3C(ch1, ch2, ch3, transMat12, transMat13);
    else
    ...
    if not(fileName == 0)
        ...
        if strcmpi(slitOrientation,'Full frame') && strcmpi(channelNumber, '3 channels')
            save(calibrationFilePath, 'transMat12', 'transMat13', 'posRect');
        else
...
% load left and right summary image from the movie
    ...
    % every nth frame
    if strcmpi(slitOrientation, 'Full frame') && strcmpi(varargin{1}, '2 channels');
    ...
    elseif strcmpi(slitOrientation, 'Full frame') && strcmpi(varargin{1}, '3 channels');
        imgLeft = double(raw.readImage(1));
        if varargin{2} == 2;
            imgRight = double(raw.readImage(2));
            for i = 5:3:raw.imageCount
                imgRight = imgRight + double(raw.readImage(i));
            end
        elseif varargin{2} == 3;
            imgRight = double(raw.readImage(3));
            for i = 6:3:raw.imageCount
                imgRight = imgRight + double(raw.readImage(i));
            end
        end
...
function fieldOfView = selectFieldOfView3C(ch1, ch2, ch3, tform12, tform13)
% show left and transformed right image overlayed and select the field of view
    fig = figure( ...
        'MenuBar', 'none', ...
        'Toolbar', 'none', ...
    ...
    imageCh2Transformed = imtransform(ch2, tform12, ...
        'XData', [1, size(ch1, 2)], ...
        'YData', [1, size(ch1, 1)]);
    imageCh2Transformed = imageNormalizeAdjust(imageCh2Transformed);
    
    imageCh3Transformed = imtransform(ch3, tform13, ...
        'XData', [1, size(ch1, 2)], ...
        'YData', [1, size(ch1, 1)]);
    imageCh3Transformed = imageNormalizeAdjust(imageCh3Transformed);
    ...
    % well aligned points should appear as a single yellow point
    % bad aligned points show up as points with the two distinct colors
    overlayedImages = zeros(size(ch1, 1), size(ch1, 2), 3, ...
                            'like', ch1);
    overlayedImages(:, :, 1) = ch1;
    overlayedImages(:, :, 2) = imageCh2Transformed;
    overlayedImages(:, :, 3) = imageCh3Transformed;
