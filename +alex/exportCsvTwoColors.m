function exportCsvTwoColors(filePath, movie, mapping, traces, selectedFrames)
% export all data from the movie and the traces as a csv file
    ...
    acceptorRawSignals = tracesEmissions(traces, 'Aem', 'raw');
    acceptorBackgrounds = tracesEmissions(traces, 'Aem', 'background');
    acceptorBackgroundCorrectedSignals = tracesEmissions(traces, 'Aem', 'corrected');
    donorRawSignals = tracesEmissions(traces, 'Dem', 'raw');
    donorBackgrounds = tracesEmissions(traces, 'Dem', 'background');
    donorBackgroundCorrectedSignals = tracesEmissions(traces, 'Dem', 'corrected');
    ...
    csvwrite(fullfile(p, [n, '-acceptorRaw', ext]), acceptorRawSignals); 
    csvwrite(fullfile(p, [n, '-acceptorBackground', ext]), acceptorBackgrounds); 
    csvwrite(fullfile(p, [n, '-acceptorBackgroundCorrected', ext]), acceptorBackgroundCorrectedSignals);
    csvwrite(fullfile(p, [n, '-donorRaw', ext]), donorRawSignals); 
    csvwrite(fullfile(p, [n, '-donorBackground', ext]), donorBackgrounds); 
    csvwrite(fullfile(p, [n, '-donorBackgroundCorrected', ext]), donorBackgroundCorrectedSignals);
end
