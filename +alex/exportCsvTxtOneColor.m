function exportCsvTxtOneColor(filePath, movie, mapping, traces, selectedFrames)
% export all data from the movie and the traces as a csv file
    
    if isempty(movie)
        error('alex:ExportTxt:NoMovie', 'movie is empty')
    end
    if isempty(traces)
        error('alex:ExportTxt:NoTraces', 'the list of traces is empty')
    end
    ...
    rawSignals = cell2mat(tracesEmissions(traces, 'raw'));
    backgrounds = cell2mat(tracesEmissions(traces, 'background'));
    backgroundCorrectedSignals = cell2mat(tracesEmissions(traces, 'corrected'));
    
    % create one header file and export it as csv
    % and one file each for every channel
    % export intensities either raw, background or backgrund corrected in
    % txt
    [p, n, ext] = fileparts(filePath);
    alex.utilities.cell2csv(fullfile(p, [n, '-header.csv']), header);
    dlmwrite(fullfile(p, [n, '-raw', ext]), rawSignals); 
    dlmwrite(fullfile(p, [n, '-background', ext]), backgrounds); 
    dlmwrite(fullfile(p, [n, '-backgroundCorrected', ext]), backgroundCorrectedSignals);
end
