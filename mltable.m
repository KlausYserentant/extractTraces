...
set(data.axes, 'box', 'on', 'DrawMode', 'fast', 'HitTest', 'off');
...
for j = 1:data.numRows
	...
	if data.doCheck % put checkboxes to left of row
		poschk = [ pos_ax(1) - chkdx...
                 ...
                 10 10 ];
		data.checks(j) = ...
          uicontrol('style','checkbox','units','pixels','position', poschk, 'Value',1);
...
% ------------------------------------------------------------------------
    ...
    % Checks suplementary Row
    if mod(row+data.ind0,2) == 1
        setCheck(fig, hObj, row+1+data.ind0, vchk)
    else
        setCheck(fig, hObj, row-1+data.ind0, vchk)
    end
return
...
% ------------------------------------------------------------------------
    ...
    if 0%row - data.ind0 <= data.numRows
